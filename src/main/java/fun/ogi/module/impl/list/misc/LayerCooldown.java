...
import fun.ogi.events.render.Event3DRender;
...
@ModuleInformation(moduleName = "Layer Cooldown", moduleDesc = "Shows plast and trap cooldown timers", moduleCategory = ModuleCategory.MISC)
public class LayerCooldown extends Module {
    ...
    public void onPacket(PacketEvent event) {
        if (event.getType() != PacketEvent.Type.RECEIVE || mc.world == null || mc.player == null) return;
    ...
    public void onHud(EventHud event) {
        ...
        for (int i = 0; i < timers.size(); i++) {
            ...
            drawTimer(event.getDrawContext(), (float) screen.x, (float) screen.y, seconds);
        }
    }
    ...
    private void drawTimer(DrawContext context, float x, float y, float seconds) {
        ...
        int accentColor = ThemeManager.getInstance().getPrimary();
        int bgColor = (accentColor & 0xFFFFFF) | (120 << 24);
        ...
        Builder.rectangle()
                ...
                .color(new QuadColorState(new java.awt.Color(bgColor, true)))
                .radius(new QuadRadiusState(3))
                .build()
                .render(matrix, boxX, boxY, 0);

        drawItemIcon(context, boxX + 4.0f, boxY + 1.25f, 0.62f);

        context.drawText(mc.textRenderer, text, (int) (boxX + 4.0f + iconSize + gap), (int) (boxY + 3.55f), textColor, true);
    }
    ...
    private void drawItemIcon(DrawContext context, float x, float y, float scale) {
