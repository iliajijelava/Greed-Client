...
@ModuleInformation(moduleName = "Particles", moduleDesc = "Beatiful particles when doing something", moduleCategory = ModuleCategory.RENDER)
public class Particle extends Module {

    public static Particle INSTANCE = new Particle();
    private static final Identifier STAR_TEXTURE = Identifier.of("cheap", "textures/particle/star.png");
    private static final Identifier HEART_TEXTURE = Identifier.of("cheap", "textures/particle/star.png");
    private static final Identifier DOLLAR_TEXTURE = Identifier.of("cheap", "textures/particle/dollar.png");
    private static final Identifier BLOOM_TEXTURE = Identifier.of("cheap", "textures/particle/bloom.png");
    private static final Identifier SPARKLE_TEXTURE = Identifier.of("cheap", "textures/particle/sparkle.png");

    private final ModeSetting type = new ModeSetting("Particle type", this,"Stars", "Stars",
            "Hearts", "Dollars", "Blum", "Glows");

    private final ListSetting reason = new ListSetting("Add when: ", this, "Doing nothing", "Running", "Hitting", "Pearl throwing","Trident throwing","Totem");

    private final SliderSetting count = new SliderSetting("Amount ", this,10f, 2f, 40f, 1);
        ...
        if (reason.isSelected("Hitting")) {
            ...
            if (target != null) {
                for (int i = 0; i < 35; i++) {
                    ...
                    addParticle(targetX, targetY, targetZ, velocity, ThemeManager.getInstance().getPrimary(), 0.3f, life, 0.5f, 0.0007f);
                }
            }
        }
    }
    ...
    public void onPacket(PacketEvent e) {
        ...
        if (!reason.isSelected("Totem")) return;
    ...
    public void onUpdate(EventUpdate e) {
        ...
        int particleCount = (int) count.getIntValue();

        if (reason.isSelected("Doing nothing")) {
            ...
            for (int i = 0; i < particleCount; i++) {
                ...
                addParticle(spawnX, spawnY, spawnZ, velocity, ThemeManager.getInstance().getPrimary(), 0.3f, life, 3.0f, 0.00005f);
            }
        }

        if (reason.isSelected("Running") && isMoving()) {
            ...
            if (!isPositionInBlock(new Vec3d(posX, posY, posZ))) {
                ...
                long life = (long) random(1500, 2000);
                addParticle(posX, posY, posZ, velocity, ThemeManager.getInstance().getPrimary(), 0.3f, life, 3.0f, 0.00005f);
            }
        }

        boolean trackPearls = reason.isSelected("Pearl throwing");
        boolean trackTridents = reason.isSelected("Trident throwing");
    ...
    private void createProjectileParticles(Vec3d position, int cnt) {
        int particleColor = ThemeManager.getInstance().getPrimary();
    ...
    public void onRender3D(EventWorldRenderer e) {
        ...
        for (ParticleData particle : renderList) {
            ...
            matrices.push();
            matrices.translate(particle.position.x - cam.x, particle.position.y - cam.y, particle.position.z - cam.z);
