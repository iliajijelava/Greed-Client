...
@ModuleInformation(moduleName = "Skeleton Esp", moduleDesc = "Renders player skeleton lines", moduleCategory = ModuleCategory.RENDER)
public class SkeletonEsp extends Module {

    private final BooleanSetting renderSelf = new BooleanSetting("Render Self", this, true);
    ...
    public void onWorldRender(EventWorldRenderer e) {
        ...
        for (Entity entity : mc.world.getEntities()) {
            ...
            render(ms, p, pt, e.getCamera().getPos());
        }
    ...
    private void render(MatrixStack ms, PlayerEntity p, float t, Vec3d cam) {
        if (p == mc.player && mc.options.getPerspective().isFirstPerson()) return;
        if (p == mc.player && !renderSelf.getValue()) return;

        double x = MathHelper.lerp(t, p.prevX, p.getX()) - cam.x;
        double y = MathHelper.lerp(t, p.prevY, p.getY()) - cam.y;
        double z = MathHelper.lerp(t, p.prevZ, p.getZ()) - cam.z;
        ...
        boolean isFriend = Cheap.getInstance().getFriendManager().contains(p.getName().getString());
        Color c = isFriend ? new Color(100, 255, 100) : new Color(ThemeManager.getInstance().getPrimary());
