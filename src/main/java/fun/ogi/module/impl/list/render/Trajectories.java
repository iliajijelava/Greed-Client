...
import fun.ogi.module.theme.ThemeColorPalette;
...
@ModuleInformation(moduleName = "Trajectories", moduleDesc = "Shows trajectories", moduleCategory = ModuleCategory.RENDER)
public class Trajectories extends Module {
    ...
    private static final Identifier GLOW_TEXTURE = Identifier.of("cheap", "textures/trajectories/glow.png");

    private final SliderSetting lineWidth = new SliderSetting("Line width",this, 2.2f, 0.5f, 5.0f, 0.1f);
    ...
    @Subscribe
    public void onRenderWorld(EventWorldRenderer event){
        ...
        Vec3d startPos = mc.player.getEyePos()
                .add(mc.player.getRotationVec(tickDelta).multiply(0.16));
        ...
        for (Vec3d direction : directions) {
            ...
            System.out.println(result.points.length);
        ...
        int themeColor = ThemeManager.getInstance().getPalette().getPrimary();
        int themeSecondaryColor = ThemeManager.getInstance().getPalette().getSecondary();
        ...
        for (int i = 0; i < resultCount; i++) {
            ...
            drawTrajectoryLine(matrix, result.points, themeSecondaryColor, cam);

            if (result.entityHit != null && result.entityHit.isAlive()) {
                drawEntityBox(matrix, result.entityHit, themeSecondaryColor, cam);
            } else if (result.blockHit != null) {
                drawImpactMarker(matrix, result.hitPos, result.blockHit.getSide(), themeSecondaryColor, cam);
            }

            if (stack.isOf(Items.SPLASH_POTION) && result.hitPos != null) {
                drawPotionRadiusGlow(matrices, result.hitPos, themeColor, cam);
            }
        }
    ...
    private void drawTrajectoryLine(Matrix4f matrix, Vec3d[] points, int color, Vec3d cam) {
        int r = (color >> 16) & 0xFF;
        int g = (color >> 8) & 0xFF;
        int b = color & 0xFF;
        int a = (color >> 24) & 0xFF;
    ...
    private void drawImpactMarker(Matrix4f matrix, Vec3d pos, Direction side, int color, Vec3d cam) {
        ...
        Vec3d center = pos.subtract(cam).add(normal.multiply(0.004));
        double radius = 0.35;
    ...
    private void drawEntityBox(Matrix4f matrix, Entity entity, int color, Vec3d cam) {
    ...
    private void drawPotionRadiusGlow(MatrixStack matrices, Vec3d pos, int themeColor, Vec3d cam) {
        int color = ThemeManager.getInstance().getPalette().getPrimary();
        int r = (color >> 16) & 0xFF;
        int g = (color >> 8) & 0xFF;
        int b = color & 0xFF;
        int a = (color >> 24) & 0xFF;
        ...
        matrices.push();
        matrices.translate(pos.x - cam.x, pos.y + 0.012 - cam.y, pos.z - cam.z);
        Matrix4f matrix = matrices.peek().getPositionMatrix();
        BufferBuilder buffer = Tessellator.getInstance().begin(
                VertexFormat.DrawMode.LINES,
                VertexFormats.POSITION_COLOR
