...
public class IdealHitUtils {
    ...
    public static boolean cooldownIsReached(boolean toSprinting) {
        ...
        float progress = 0f;
        if (mc.gameRenderer.firstPersonRenderer != null) {
            progress = ((HeldItemRendererInvoker) mc.gameRenderer.firstPersonRenderer).getEquipProgressMainHand();
        }
        ...
        return mc.player.getAttackCooldownProgress(0.5f) >= 0.93f;
    }

    public static float getEquipProgress() {
        if (mc.gameRenderer.firstPersonRenderer == null) return -1f;
        return ((HeldItemRendererInvoker) mc.gameRenderer.firstPersonRenderer).getEquipProgressMainHand();
    }
    ...
    public static boolean canCritical() {
        ...
        boolean notCrit = mc.player.isInLava()
                ...
                || (Cheap.getInstance().getModuleStorage().get(FreeCam.class) != null && Cheap.getInstance().getModuleStorage().get(FreeCam.class).isEnabled())
                || mc.player.isOnGround() && !mc.options.jumpKey.isPressed() && !Cheap.getInstance().getModuleStorage().get(AttackAura.class).onlySpace.getValue();
    ...
    private boolean isNoJumpDelayCeilingCritIntent() {
        NoDelay noDelay = Cheap.getInstance().getModuleStorage().get(NoDelay.class);
        return noDelay != null
                && noDelay.isEnabled()
                && mc.options != null
