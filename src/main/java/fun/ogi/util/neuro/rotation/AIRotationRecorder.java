...
public class AIRotationRecorder {
    ...
    public void onTick(EventUpdate event) {
        ...
        LivingEntity target = AttackAura.lastTarget;
        if (target == null || !target.isAlive()) return;

        AttackAura attackAura = Cheap.getInstance().getModuleStorage().get(AttackAura.class);
        if (attackAura == null || !attackAura.isEnabled()) return;
        ...
        double distance = attackAura.distance.getValue();
        Vec3d targetPoint = BestPoint.getMultipoint(target, distance);
        Rotation targetRotation = new Rotation(new RotationUtil().calculate(targetPoint));
