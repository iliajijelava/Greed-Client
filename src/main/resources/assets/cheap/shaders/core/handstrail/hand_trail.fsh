...
// Feedback-шлейф (порт hand_trail под ShaderProgram-путь Wyvern).
...
void main() {
    ...
    // Репроекция: компенсируем поворот камеры, иначе накопленный след «прилипает»
    // к экрану и мажет весь кадр при движении мышью.
    vec2 prevUv = texCoord + camShift;
    prevUv += curl * texelSize * (1.9 + blurRadius * 1.35);

    // Idle-«плавание»: когда руки не бьют и не двигаются, медленно гоним накопленный
    // шлейф в стороны единым ровным сдвигом, чтобы он красиво «плыл» цельным шлейфом,
    // а не дёргался турбулентностью. Фаза общая для всего кадра — без зависимости от UV.
    ...
    // За пределами кадра прошлого шлейфа нет — гасим, чтобы не тянуть края.
